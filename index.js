...
d3.csv('aldiwan.csv',function(data){
    ...
    poets = d3.nest().key(d=>d.poet).entries(data)
    count = poets.map(function(d){ 
        return {'name': d.key,
                'era':d.values[0]['era'], 
                'len': d.values.filter(dd => dd.poem_flag=='poem').length}})

    nodes = count.map(function(p){
        era = p.era
        era_origin = p.era
    ...
    function poetSummary(poet){
       ...
       var chart_tooltip = d3.select("body")
        ...
        poet_df = df.filter(d=>d.poet==poet)
        var beat_df = d3.nest().key(d=>d.beat).entries(poet_df)
        beat_breakdown = beat_df.map(function(d){
                return {'beat'  : d.key,
                        'poems' : d.values.filter(dd=>dd.poem_flag=='poem').length
                        }
            }).filter(d=>d.beat!='')
        ...
        genres = d3.nest().key(d=>d.genre).entries(poet_df)
        genres = genres.map(function(d){
            return {genre: d.key, 
                    poems: d.values.length}
            })
       ...
       var poet_info =  poet_page.append('div')
        ...
        quotes = poet_df.filter(d=>d.poem_flag=='quote')
        if (quotes.length > 0){
            poet_info
                .append('div')
                .style('padding','5px')
